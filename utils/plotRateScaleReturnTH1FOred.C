...
//added xaxis
TH1F* plotRateReturnTH1FOred(TString variable, TString variable1, float ordPt,TString fileName,  TString outFileName, TString l1Cut, TString xaxis, TString text, TString treePath, int bins, float low, float high, Color_t color, float scaledRate, bool variableBin=false, int linestyle=1, TString yaxis = "Rate"){
  ...
  TH1F* histo1 = (TH1F*)tauFile->Get(treePath + variable1);
  if(histo == 0 || histo1 == 0){
  ...
  rateHisto->GetYaxis()->SetTitleOffset(0.87);

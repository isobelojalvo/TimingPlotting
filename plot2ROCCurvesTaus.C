...
void plot2ROCCurvesTaus(TString Date, TString variableA, TString variableScanA, float variableValuesA[15], 
			...
			TString variableB, TString variableScanB, float variableValuesB[15], 
			TString fileB, TString treeSignalB, TString treeBkgdB, TString numeratorSignalB, TString denominatorSignalB, TString numeratorBkgdB, TString denominatorBkgdB, TString textB,
  ...
  for(int i = 0; i < 1; i++){
    //input variable values as strings for text
    float variableValue  = variableValuesB[i];
    std::ostringstream strs;
    strs << variableValue;
    std::string text1 = strs.str();
    variableValue = variableValuesB[i];
    //finish creating string values

    //create numeratorCut
    TString numeratorCut    = variableScanB + "<" + fToS(variableValue);

    //Get Signal Efficiency
    TString tempOutNameSig  = Date+"/sigVar-"  + fToS(variableValue) + outFileName;
    signal[i]  = plotReturnEfficiency(fileB, treeSignalB, variableB, numeratorCut +"&&"+numeratorSignalB, denominatorSignalB, tempOutNameSig,  xaxis, yaxis, text, bins, xlow, xhigh, color);

    //Get Background Efficiency
    TString tempOutNameBkgd = Date+"/bkgdVar-" + fToS(variableValue) + outFileName;
    bkgd[i]    = plotReturnEfficiency(fileB, treeBkgdB,   variableB, numeratorCut +"&&"+numeratorBkgdB , denominatorBkgdB, tempOutNameBkgd, xaxis, yaxis, text, bins, xlow, xhigh, color);

    std::cout<<"Numerator Selection "+variableB+": "<<variableValue <<" signal: "<<signal[i]<<" fake: "<<bkgd[i]<<std::endl;
  ...
  TGraph* rocB = new TGraph(14,signal,bkgd);
  rocB->SetLineColor(color);
  rocB->SetMarkerColor(color);
  rocB->SetMarkerStyle(4);
